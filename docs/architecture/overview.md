...
`/webnovel-plan` 只负责卷级规划和设定/总纲同步；它在生成前必须先与作者逐轮讨论 10 项卷级必答议题（无轮次上限，每轮带「不再讨论」选项），把结论落成决策卡 `大纲/第N卷-规划讨论.md` 并通过闸门（未决项为 0、`BLOCKER` 为 0、作者已确认），未通过时不落任何卷级产物。`/webnovel-volume-revise` 用确认式流程定向修改已有卷纲（复用同一套有限选项与「不再讨论」话术），`/webnovel-volume-reload` 负责人工编辑后的内容 revision 重载。revision 变化会把依赖旧卷纲的章纲登记为 `stale`，但不会覆盖作者文件。卷纲修改只演进**尚无章纲覆盖**的卷级内容：已被已存在章纲落实的部分是既定事实（**锁定区**），不得改写——正文经由章纲与卷纲关联，章纲锁定即意味着正文同样不动。锁定区以已存在章纲为基准，比 `/webnovel-outline-revise` 以 accepted commit 为基准的冻结线更宽（锁定区 ⊇ 冻结线）。

`/webnovel-chapter-plan` 在卷级产物齐全后按批次生成或刷新独立章纲与章节合同，同样采用「先对话后生成」：每个批次先讨论 10 项章级必答议题（批次切分与范围、卷级节拍节点到章号的映射、关键章目标、阻力与代价递增顺序、CEN → CBN 承接链、时间锚点与章内跨度、倒计时节奏、伏笔埋收、爽点与 Strand 分布、本批禁区），把结论累积到决策卡 `大纲/第N卷-章纲规划讨论.md`，未决项不为 0 时不落任何章级产物；首批问全 10 项，后续批次只问未决项与偏离项。`/webnovel-chapter-revise` 复用同一套确认话术定向修改已有章纲，并在修改后重刷章级合同。已有章纲或正文的章在 `chapter-plan` 中被一律跳过、逐字节不动——改章纲只有 `chapter-revise` 一条路。卷纲完成本身不代表章节已经具备写作条件。

卷纲 revision 以节拍表、时间线、详细大纲三份文件的名称与内容 SHA-256 为依据。写前门禁会比较章节记录的 `source_volume_revision` 与当前 revision；不一致时先重载卷纲，再按锁定区分流：已有章纲的章零动作（章纲调整走 `/webnovel-chapter-revise`），尚无章纲的章才重新规划。`volume-reload` 的输出同时给出 `protected_chapters`（锁定区，本次未修改）与 `open_chapters`（可改区）及作用域声明。

### 四层方向透传

总纲 → 卷纲 → 章纲 → 正文是一条单向链：总纲指导卷纲，卷纲指导章纲，章纲指导正文。任一层变化时都要检查与紧邻上下层是否同向；不同向时由作者裁决改哪一侧，裁决结果逐级上溯。三个节点与统一规律（**每层只有「最后一个单位」可改**）：
...
裁决三值跨节点通用：`align_downstream`（改产物）、`align_upstream`（改依据，须先上溯）、`accepted_deviation`（只记录）；章-正节点的既有章级值 `outline_to_body` / `body_to_outline` 是一/二两值的别名，读入时归一。CLI 入口是 `handoff --node {node} [--target {n}] [--check|--record]`，裁决记录只写 `.story-system/handoffs/`，不改任何创作文件。
