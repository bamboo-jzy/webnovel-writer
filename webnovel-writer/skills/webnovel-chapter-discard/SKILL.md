...
description: 抛弃一章正文。未提交草稿直接删除并归档可恢复；已提交章回退到上一章版本点。只处理最后一章。
...
- **已提交章**（accepted）**不做外科手术式删除**。`state.json` 的 `plot_threads` / `strand_tracker` / `protagonist_state` / `world_settings` 是累计字段，`index.db` 的 `entities.first_appearance` / `last_appearance` 明确不随撤回回退——只删正文与投影行会留下一本「读模型说没写过、状态却记得写过」的书。已提交章一律走版本点回退。
- 版本点回退会同时还原 `正文/`、`.story-system/commits/`、`state.json`、`index.db`、`summaries/`、`projection_log`。它**不删除任何提交**：原分支仍指向被抛弃的那次提交，随时可取回。
- **章号 off-by-one**：`chNNNN` 的语义是「第 N 章**完成后**」。抛弃第 N 章要回退到 `ch{N-1}`，不是 `chN`。第 1 章没有 `ch0000`，回退目标是仓库初始提交（分支名 `rewrite-from-start`）。
...
- 回退成功后当前分支会变成 `rewrite-from-chXXXX`；后续写章、备份都发生在这个分支上，无需切回原分支（原分支保留着被抛弃的提交）。
...
向作者展示：`plan.classification`（`draft` / `accepted` / `absent`）、正文路径、`plan.downstream_chapters`、`plan.planned_actions`；已提交章还要展示 `plan.rollback.command`、`plan.rollback.recovery_hint` 与将被还原的目录清单。
...
- `rollback_branch_exists`：同名回退分支已存在，先处理它；
...
CLI 只在预览的全部检查通过后执行 `git switch -c rewrite-from-ch{NNNN} ch{NNNN}`，并做三项回退后核对：`head_matches_target`、`chapter_body_absent`、`current_chapter_after`。任一项不符会返回 `ok=false`，此时**停下并让作者手工 `git status` 确认**，不要自行追加 git 命令。
...
报告实际动作、归档目录或回退分支、`current_chapter` 变化、章纲是否保留、以及「下一章仍是第 N 章、可直接用 `/webnovel-write` 重写」，不输出 traceback 或长日志。

被抛弃的正文可用 `git show <原分支>:正文/第N章-*.md` 取回核对；不要为了让工作树干净而手工删除 `正文/` 或 `.story-system/commits/` 里的文件。
